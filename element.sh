...
if [[ $1 ]] 
  ...
  then
    ...
    echo $ELEMENT | while read ATOM_NUM BAR ATOM_MASS BAR MELT BAR BOIL BAR ELEM BAR NAME BAR TYPE 
    do
      echo "The element with atomic number $ATOM_NUM is $NAME ($ELEM). It's a $TYPE, with a mass of $ATOM_MASS amu. $NAME has a melting point of $MELT celsius and a boiling point of $BOIL celsius."
    done
  fi
else
